...
exports.AuthStateManager = exports.DEFAULT_AUTH_STATE = void 0;
...
exports.DEFAULT_AUTH_STATE = DEFAULT_AUTH_STATE;
...
const isSameAuthState = (prevState, state) => {
  return prevState.isPending === state.isPending && prevState.isAuthenticated === state.isAuthenticated && JSON.stringify(prevState.idToken) === JSON.stringify(state.idToken) && JSON.stringify(prevState.accessToken) === JSON.stringify(state.accessToken) && prevState.error === state.error;
};
...
class AuthStateManager {
  ...
  updateAuthState() {
    ...
    const {
      autoRenew,
      autoRemove
    } = this._sdk.tokenManager.getOptions();
    ...
    const cancelablePromise = new PCancelable((resolve, _, onCancel) => {
      ...
      this._sdk.tokenManager.getTokens().then(({
        accessToken,
        idToken,
        refreshToken
      }) => {
