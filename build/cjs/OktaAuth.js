...
var _startAuthTransaction = require("./idx/startAuthTransaction");
...
class OktaAuth {
  constructor(args) {
    ...
    if ((0, features.isBrowser)()) {
      ...
      this.userAgent = (0, _builderUtil.getUserAgent)(args, `okta-auth-js/${"5.0.1"}`);
    } else {
      this.userAgent = (0, _builderUtil.getUserAgent)(args, `okta-auth-js-server/${"5.0.1"}`);
    ...
    this.idx = {
      ...
      startAuthTransaction: _startAuthTransaction.startAuthTransaction.bind(null, this)
    }; // Fingerprint API
  ...
  async signIn(opts = {
    ...
    if (useInteractionCodeFlow) {
      return (0, _idx.authenticate)(this, opts);
    } // Authn V1 flow
  ...
  resetPassword(opts) {
    return (0, _tx.postToTransaction)(this, '/api/v1/authn/credentials/reset_password', opts);
  } // { username, (relayState) }
