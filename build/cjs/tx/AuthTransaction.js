...
function link2fn(sdk, res, obj, link, ref) {
  if (Array.isArray(link)) {
    ...
    switch (method) {
      case 'GET':
        return function () {
          return _http.default.get(sdk, link.href, {
...
function links2fns(sdk, res, obj, ref) {
  ...
  for (var linkName in obj._links) {
    ...
    var link = obj._links[linkName]; // replace responses hostname with issuer hostname provided by SDK

    if (Object.prototype.hasOwnProperty.call(link, 'href')) {
      var matchProtocolDomainHost = /^.*\/\/[^\/]+:?[0-9]?/i; // eslint-disable-line no-useless-escape

      var sdkProvidedissuedDomainHost = link.href.replace(matchProtocolDomainHost, sdk.getIssuerOrigin());
      Object.assign(link, {
        href: sdkProvidedissuedDomainHost
      });
    }
